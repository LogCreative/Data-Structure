...
public:
    ...
    elemType getBack() const {return rear->data;}
...
public:
    ...
    // 最短路径 + 字典序最小
    void OutputPath(){
        int* distance = new int[levelEnd[levelLineNum]+1];
        int* prev = new int[levelEnd[levelLineNum]+1];
        ...
        while(!path.isEmpty()){
            ...
            int v;
            for(int i = 0;i < 4;++i){
                if(isAvaliable(u,v,i)&&Find(u)==Find(v)){
                    if(distance[v]==-1||distance[u]+1<distance[v]){
                        distance[v]=distance[u]+1;
                        prev[v]=u;
                        path.enQueue(v);
                    } else if (distance[u]+1==distance[v]&&path.getBack()>v){
                        path.pop_back();
                        prev[v]=u;
                        path.enQueue(v);
                    }
